...
async Registrarse(user){
  ...
  console.log(API_URL)
  await axios.post(API_URL + '/api/Usuario/register',user).then((response)=>{
...
this.setState((PrevState)=>({
  ...
  console.log(user)
  await axios.post(API_URL + '/api/Usuario/login',user).then( async (response)=>{
...
CrearNota=async(Obj)=> {
  ...
  console.log("entre, el obj es: " + Obj)
  await axios.post(API_URL + '/api/Recordatorio?idUsuario='+this.state.user.id,{titulo:Obj.titulo,
  descripcion:Obj.descripcion,prioridad:Obj.prioridad,fecha:Obj.fecha+"T"+Obj.hora+":37.775Z"}).then((response)=>{
    if(response.data.succeeded){
      ...
      console.log("actualice el estado2")
    }
})
...
ActualizarNota=async(Obj,id)=> {
  this.setState((PrevState)=>({
    ...PrevState,isLoad:true
      ...
      console.log(this.state)
      var nts = this.state.user.notas
      await axios.put(API_URL + '/api/Recordatorio',{titulo:Obj.titulo,
...
EliminarNota=async(id)=>{
  this.setState((PrevState)=>({
    ...
    await axios.delete(API_URL + "/api/Recordatorio/"+id.toString()).then((response)=>{
