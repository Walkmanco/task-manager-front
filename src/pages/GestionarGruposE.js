...
class GestionarGruposE extends Component{
    ...
    async AgregarGrupo(id){
        ...
        console.log(this.context.GlobalState)
        await axios.post(API_URL + "/api/Grupo/"+id+"/"+this.context.GlobalState.user.id,{}).then((response)=>{
    ...
    async EliminarGrupo(id){
        var grp = this.state.grupos
        console.log(this.context.GlobalState)
        await axios.delete(API_URL + "/api/Grupo/"+id+"/usuarios/"+this.context.GlobalState.user.id).then((response)=>{
